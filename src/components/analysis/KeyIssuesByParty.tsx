...
// inverse question from the Age/Gender/Religion x Party sections further
// down the page (which ask "of women, X% support BJP"). Each cell's own
// count already cleared (or failed) the privacy threshold server-side; a
...
function CompositionList({ cells, suppressedLabel }: { cells: CompositionCell[]; suppressedLabel: string }) {
  ...
  return (
    <ul className="space-y-1.5">
      {cells.map((cell) => (
        <li key={cell.key} className="flex items-center gap-2 text-xs">
...
// party's own supporter count (never total issue mentions). The dropdown
// lets a visitor focus on one party at a time as a donut (matching the
// overall Key Issues section's visual language); the compact grid below
// still shows every party at a glance for quick comparison. A party whose
// supporter count is below the privacy/sample threshold shows the honest
// "not enough data" state instead of a donut built from too few people.
export function KeyIssuesByParty({ parties }: { parties: PartyTopIssues[] }) {
  const { t, locale } = useLocale();
  const [selectedKey, setSelectedKey] = useState(parties[0]?.partyKey ?? "");

  const selected = parties.find((p) => p.partyKey === selectedKey) ?? parties[0];
  ...
  if (parties.length === 0 || !selected) {
    return <p className="text-sm text-muted">{t.analysisHub.notEnoughPartyIssueData}</p>;
  }

  const selectedName = locale === "hi" && selected.partyNameHindi ? selected.partyNameHindi : selected.partyLabel;

  return (
    <div>
      <label className="block max-w-xs">
        <span className="mb-1.5 block text-xs font-semibold text-foreground">{t.analysisHub.selectPartyLabel}</span>
        <select value={selectedKey} onChange={(e) => setSelectedKey(e.target.value)} className={SELECT_CLASSNAME}>
          {parties.map((p) => (
      ...
      <div className="card-surface mt-4 rounded-2xl p-4 sm:p-5">
        ...
        {(selected.genderComposition.length > 0 || selected.ageComposition.length > 0 || selected.religionComposition.length > 0) && (
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {parties.map((party) => {
          const name = locale === "hi" && party.partyNameHindi ? party.partyNameHindi : party.partyLabel;
          return (
            <button
              key={party.partyKey}
              type="button"
              onClick={() => setSelectedKey(party.partyKey)}
              className={`card-surface min-w-0 rounded-2xl p-4 text-left transition-colors ${party.partyKey === selectedKey ? "ring-2 ring-ink/30" : "hover:bg-surface-2"}`}
            >
              <div className="flex items-center gap-2.5">
                {party.logoUrl ? (
                  <Image src={party.logoUrl} alt="" width={28} height={28} className="h-7 w-7 shrink-0 rounded-full object-contain" />
                ) : (
                  <span
                    className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold text-white"
                    style={{ background: party.colorHex ?? "var(--ink)" }}
                  >
              ...
              <div className="mt-3">
                <RankedIssueList issues={party.topIssues.slice(0, 3)} emptyLabel={t.analysisHub.notEnoughPartyIssueData} />
              </div>
            </button>
          );
        })}
